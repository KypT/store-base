...
window.UI = (function () {
    ...
    return {
        ...
        Counter: (function () {
            return {
                create: function($counter, min, max) {
                    var $dec = $counter.find('.less-amount'),
                        $inc = $counter.find('.more-amount'),
                        $val = $counter.find('.amount-counter'),
                        callback = undefined;
                    ...
                    function count(op) {
                        return function() {
                            ...
                            if (count == max) $inc.addClass('disabled');
                            if (count <  max) $inc.removeClass('disabled');
            ...
            return {
                create: function ($modal) {
                    var $modalCounter = $modal.find('.amount-control'),
                        ...
                        $productImage = $modal.find('.product-image');
                    ...
                    return {
                        ...
                        show: function (product) {
                            ...
                            history.pushState(true, null, '/store/' + product.name);

                            function nextImage() {
                                if (product.images.length <= ++imageId) imageId = 0;
                                $productImage.attr('src', product.images[imageId].file.url);
                            }
                            ...
                            if (window.Admin != undefined) {
                                var url = '/store/' + product.name;
                                $modal.find('form.file-upload-zone').attr('action', url);
                                $modal.find('*[contenteditable="true"]').attr('data-url', url);
                            }

                            var counter = UI.Counter.create($modalCounter, 1);
                            counter.click(function(counter, val) {
                                if (val >= product.stock) {
                                    modalBuyButton.addClass('disabled');
                                }
                                else {
                                    if (modalBuyButton.hasClass('disabled'))
                                        modalBuyButton.removeClass('disabled');
                                }
                            });

                            $modal.find('.img-container .left').click(prevImage);
                            $modal.find('.img-container .right').click(nextImage);
                            ...
                            $productImage.attr('src', Products.imageFor(product)).attr('alt', product.name);
...
$(function() {
    ...
    var modal = UI.initModal4Products($products, $modal);
